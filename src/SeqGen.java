...
public class SeqGen
{
    ...
    public static int noteToMidi(String note) {
        ...
        if(note.charAt(1) == '#') {
            octave_start_idx = 2;
            note_letter = note.substring(0, 2);// note.charAt(0) + note.charAt(1);
        }


        int octave_num = Integer.parseInt(note.substring(octave_start_idx));
        int note_num = java.util.Arrays.asList(NOTE_NAMES).indexOf(note_letter);


        return octave_num * 12 + note_num;
    }


    public static void writeSeq(List<String> seq, String filename) {

        SeqGen mf = new SeqGen();

        for(String note: seq) {
            int midi_note = noteToMidi(note);
            mf.noteOnOffNow (QUAVER, midi_note, 127);
        }
    ...
    /** MidiSerializer method — creates a file test1.mid when the class
     is executed */
            ...
            throws Exception
    {
        ...
        int midi_note = noteToMidi("D#10");
        System.out.println(midi_note);


        //SeqGen mf = new SeqGen();

        // MidiSerializer 1 — play a C major chord
        ...
        // MidiSerializer 2 — play a scale using noteOnOffNow
        ...
        // MidiSerializer 3 — play a short tune using noteSequenceFixedVelocity
        ...
        //mf.writeToFile ("data/test_out.mid");
    }
}